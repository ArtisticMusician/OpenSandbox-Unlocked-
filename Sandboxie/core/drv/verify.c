...
{
    // OpenSandbox modification:
    // Force a fully unlocked max-level certificate so all features
    // (including Advanced Feature Pack features) are available without
    // requiring a supporter certificate.
    ...
    Verify_CertInfo.expiration_date = -1; // Never expire
